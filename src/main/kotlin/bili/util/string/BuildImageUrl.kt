...
import me.hbj.bikkuri.bili.enums.ImageFormat
 ...
 * @param format 图片格式, [ImageFormat.UNKNOWN] 将保留链接原格式, 否则一律添加格式参数, 无论原图是否匹配
...
fun buildImageUrl(
  ...
  when (format) {
    ...
    ImageFormat.WEBP -> sb.append(".webp")
    ImageFormat.UNKNOWN -> {
      // [UNKNOWN] for keep origin link format
    }
