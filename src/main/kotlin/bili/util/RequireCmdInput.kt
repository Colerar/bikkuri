package me.hbj.bikkuri.bili.util

import me.hbj.bikkuri.utils.qualifiedOrSimple
...
internal inline fun <reified T : Number> requireCmdInputNumber(
  ...
  while (loop) {
    ...
    input = when (T::class) {
      ...
      else -> throw IllegalArgumentException("Unsupported Number Type: ${T::class.qualifiedOrSimple}")
    }.let { it as? T? }
    if (input != null) {
      loop = false
    } else {
  ...
  return input
    ?: throw IllegalArgumentException("Unable to parse input $input to ${T::class.qualifiedOrSimple}")
}
